...
def bake(skin, dst_dir=r"C:\roms\mvc2_us_skin", base_dir="."):
    """Bake a skin dict into a patched GDI copy. Returns (gdi_path, info-string)."""
    charname = skin["char"]
    lba, size, root_lba, entry_off = find_dat_full(os.path.join(SRC_DIR, TRACK), charname)
    ...
    return os.path.join(dst_dir, gdi), " · ".join(info)


def main():
    skin = json.load(open(sys.argv[1]))
    dst_dir = sys.argv[2] if len(sys.argv) > 2 else r"C:\roms\mvc2_us_skin"
