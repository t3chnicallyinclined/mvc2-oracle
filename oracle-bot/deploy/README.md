...
> **Production host: `ubuntu@15.204.141.58`** = **rise3** (OVH RISE, North America), which has
> served `nobd.net` since the 2026-09-01 cutover. Log in as `ubuntu` (passwordless sudo, key
> `~/.ssh/ovh_maplecast`) — **there is no root login on rise3**, so prefix privileged commands
> with `sudo`. Both bots run there as `nobd-oracle` / `nobd-roles` under `/opt/nobd-oracle/`.
> Previous hosts, do NOT deploy to them: `149.28.44.118` (Vultr, served nobd.net until
> 2026-09-01; still powered on) and `66.55.128.93` (**decommissioned
...
scp /tmp/oracle-bot.tgz ubuntu@15.204.141.58:/tmp/
# rise3: /opt/nobd-oracle is root-owned and there is no root login, so the remote half runs
# under sudo (ubuntu has passwordless sudo).
ssh ubuntu@15.204.141.58 'sudo bash -c '"'"'D=/opt/nobd-oracle; mkdir -p $D; tar -xzf /tmp/oracle-bot.tgz -C $D;
...
```bash
bash oracle-bot/deploy/deploy.sh ubuntu@15.204.141.58
```
- **First run** copies `.env.example`/`roles.env.example` → the VPS and stops. Fill the secrets, then start:
  ```bash
  ssh ubuntu@15.204.141.58
  sudo nano /opt/nobd-oracle/oracle-bot/discord-bot/.env      # Oracle: DISCORD_BOT_TOKEN, ANTHROPIC_API_KEY, GITHUB_TOKEN
  sudo nano /opt/nobd-oracle/oracle-bot/discord-bot/roles.env # Roles: DISCORD_BOT_TOKEN (NOBD admin) + guild/channel ids
  sudo systemctl enable --now nobd-oracle nobd-roles
  ```
- **Later runs** sync the code and `systemctl restart` both automatically.

## Healthcheck
```bash
ssh ubuntu@15.204.141.58 /opt/nobd-oracle/status.sh   # no sudo needed (verified 2026-09-02)
```
...
## Logs / control
```bash
ssh ubuntu@15.204.141.58 sudo journalctl -u nobd-oracle -f     # live logs ("Oracle bot online …")
ssh ubuntu@15.204.141.58 sudo journalctl -u nobd-roles -f      # roles bot
ssh ubuntu@15.204.141.58 sudo systemctl restart nobd-oracle nobd-roles
```
